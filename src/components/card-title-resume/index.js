...
const CardTitleResume = ({ id }) => {
  ...
  return (
    <div className='content-card-movie-resume'>
      <a href={`/watch/${id}`}>
        <div className='solo-back-resume'>
          <img
      ...
      <div className='bar'>
        <div className='rating-movie' title='Crítica Agregada'>{ratingMovie.substring(0, 3)}</div>
        {
          (UserUtilityResume?.find(movieMark => movieMark?.idMovie === id)) ?
            <div className='slowman'>
              <BsFillPlayFill onClick={goWatch} className='resume-movie-resume' />
              <p className='watch-now-title-resume-resume' onClick={goWatch} title={`Continuar ${movie.title}`}>Continue assistindo</p>
              <div className='current-percentage-background-resume'>
                <div style={{width: `${infoResume?.currentTimeMovie}%`}} className='current-percentage-resume'></div>
              </div>
            </div>
            :
            <div>
...
export default CardTitleResume;