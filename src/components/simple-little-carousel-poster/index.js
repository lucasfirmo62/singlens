...
import './styles.css';
import CardLittleTitlePoster from '../../components/card-little-title-poster';
import { MdOutlineArrowBackIosNew } from 'react-icons/md';
import { MdOutlineArrowForwardIos } from 'react-icons/md';



const CarouselLittleSimplePoster = ({ name, titles }) => {
  ...
  return (
    <div>
      ...
      <div className='content-simple-carousel'>
        <button id="low" onClick={previous} className="btnPreviousRLP">
        ...
        <ul id={carouselName} className={carouselName}>
          {movies.map((movie, index) => (
            <li className="title-item" key={index}>
              <CardLittleTitlePoster
                id={movie.id}
              />
            </li>
          ))}
        </ul>
        <button className="btnNextRLP" id="low" onClick={next}>
...
export default CarouselLittleSimplePoster;